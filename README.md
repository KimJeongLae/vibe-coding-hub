...
메인 페이지에는 참가자 이름 셀이 있고, 이름을 누르면 그 사람의 페이지로 들어갑니다.
각 참가자는 **자기 페이지 파일(`app/members/본인slug/page.tsx`)을 직접 코딩**해서
본인이 만든 작업물을 채웁니다. `git push` 하면 Vercel이 자동 배포하여 셀 페이지에 바로 반영됩니다.
(원하면 Supabase로 동적 기능을 추가할 수도 있습니다 — 선택 사항.)

**🌐 라이브 사이트:** https://vibe-coding-hub-kappa.vercel.app

## 📚 문서
- [Supabase 연결하기 (관리자용)](docs/supabase-연결.md)
- [수업 전날 관리자 체크리스트](docs/관리자-체크리스트.md)
- [직원용 가이드 — 내 페이지 만들기](docs/직원-가이드.md)
